...
import { useRegisterUserClients } from "@/application/hooks/useRegisterUserClients";
...
export default function RegistrationFormReceptional() {
  const {
    register,
    handleSubmit,
    control,
  ...
  const { mutate, isPending, isSuccess, error, isError } = useRegisterUserClients({
    onSuccess: () => {
  ...
  const onSubmit: SubmitHandler<RegistrationFormValues> = (data) => {
    const payload: RegistrationFormClientsReceptForm = {
       id: session?.user?.id ?? "", 
