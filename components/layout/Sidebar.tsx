...
const personnelNavItems = [
  ...
  { href: "/schedule/exeption", label: "Schedule Exception" },
  { href: "/clients", label: "Client Appointments" },
...
export default function Sidebar({ isOpen, closeMenu, userRole }: SidebarProps) {
  ...
  const translatedNavItems = navItems.map(item => ({
    ...item,
    label: t(item.label.toLowerCase().replace(/\s+/g, ''), item.label)
  }));
      ...
      >
        <div className="md:hidden flex justify-between items-center">
          <div className="text-xl font-bold mb-4 select-none cursor-default">
            {t('sidebarLogoText', 'HabariDoc')}
          </div>

          <button
            className="self-end mb-4 p-1 rounded-md hover:bg-gray-200 cursor-pointer"
            onClick={closeMenu}
            aria-label={t('sidebarCloseButtonLabel', 'Close sidebar')}
          >
            <X size={24} />
          </button>
        </div>
